...
IPlugChunks::IPlugChunks(const InstanceInfo& info) : Plugin(info, Config(kNumParams, kNumPresets)), mStepPos(0)
{
			...
			->SetActionFunction([pGraphics](IControl* pCaller) {
				...
				if (lastHit > -1)
				{
					...
					pGraphics->GetDelegate()->SendArbitraryMsgFromUI(
						kMsgTagSliderChanged, kCtrlMultiSlider, sizeof(ParamTuple), &data);
				}
			});
	};
#endif
...
bool IPlugChunks::OnMessage(int msgTag, int ctrlTag, int dataSize, const void* pData)
{
	if (msgTag == kMsgTagSliderChanged)
	{
		...
		mSteps[pConverted->idx] = static_cast<float>(pConverted->value);
...
void IPlugChunks::OnIdle()
{
	if (mStepPos != mPrevPos)
	{
		...
		SendControlMsgFromDelegate(
			kCtrlMultiSlider, IVMultiSliderControl<>::kMsgTagSetHighlight, sizeof(int), &mPrevPos);
	}
}
