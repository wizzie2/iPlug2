...
{
 public:
	Font(const char* fontName, const char* fontStyle) : PlatformFont(true), mDescriptor {fontName, fontStyle} {}
...
{
 public:
	MemoryFont(const char* fontName, const char* fontStyle, const void* pData, int dataSize) : Font(fontName, fontStyle)
	{
		mSystem = false;
...
void IGraphicsWeb::PromptForFile(WDL_String& filename, WDL_String& path, EFileAction action, const char* ext)
{
	...
	// inputEl.call<void>("addEventListener", std::string("input"), val::module_property("file_dialog_callback"),
	// false); inputEl.call<void>("addEventListener", std::string("onChange"),
	// val::module_property("file_dialog_callback"), false);
}
...
bool IGraphicsWeb::PromptForColor(IColor& color, const char* str, IColorPickerHandlerFunc func)
{
	...
	inputEl.call<void>(
		"addEventListener", std::string("onChange"), val::module_property("color_picker_callback"), false);
