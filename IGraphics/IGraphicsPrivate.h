...
class APIBitmap
{
 public:
	 ...
	 * @param scale An integer representing the scale of this bitmap in relation to a 1:1 pixel screen, e.g. 2 for an
	 * @2x bitmap
	...
	APIBitmap() : mBitmap(0), mWidth(0), mHeight(0), mScale(0), mDrawScale(1.f) {}
...
class IFontInfo
{
 public:
	...
	const WDL_String& GetFamily() const
	{
		return mFamily;
	}
	const WDL_String& GetStyle() const
	{
		return mStyle;
	}
	...
	double GetHeightEMRatio() const
	{
		return static_cast<double>(mUnitsPerEM / (mAscender - mDescender));
	}  // TODO: codereview
 ...
 private:
	...
	{
		return mData[loc + 0] == tag[0] && mData[loc + 1] == tag[1] && mData[loc + 2] == tag[2] &&
			   mData[loc + 3] == tag[3];
	}
				...
				languageID == GetUInt16(loc + 4) && nameID == GetUInt16(loc + 6))
			{
				...
				switch (stringID)
				{
					case EStringID::Windows:
					{
						...
						conv.out(mbs,
								 utf16.Get(),
								 utf16.Get() + utf16.GetSize(),
								 a,
								 utf8.Get(),
								 utf8.Get() + utf8.GetSize(),
								 b);
	...
	uint16_t GetUInt16(uint32_t loc)
	{
		return (((uint16_t) mData[loc + 0]) << 8) | (uint16_t) mData[loc + 1];
	}
	int16_t GetSInt16(uint32_t loc)
	{
		return (((uint16_t) mData[loc + 0]) << 8) | (uint16_t) mData[loc + 1];
	}
	uint32_t GetUInt32(uint32_t loc)
	{
		return (((uint32_t) GetUInt16(loc + 0)) << 16) | (uint32_t) GetUInt16(loc + 2);
	}
	int32_t GetSInt32(uint32_t loc)
	{
		return (((uint32_t) GetUInt16(loc + 0)) << 16) | (uint32_t) GetUInt16(loc + 2);
	}
#else
...
class PlatformFont
{
 public:
	...
	virtual FontDescriptor GetDescriptor()
	{
		return nullptr;
	}
	virtual IFontDataPtr GetFontData()
	{
...
struct SVGHolder
{
	SVGHolder(sk_sp<SkSVGDOM> svgDom) : mSVGDom(svgDom) {}
...
struct SVGHolder
{
	SVGHolder(NSVGimage* pImage) : mImage(pImage) {}
