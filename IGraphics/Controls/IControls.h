...
{
 public:
	IVSwitchControl(const IRECT& bounds,
...
class IVTabSwitchControl : public ISwitchControlBase, public IVectorBase
{
 public:
	 ...
	 * @param options An initializer list of CStrings for the button labels to override the parameter display text
	 * labels. Supply an empty {} list if you don't want to do that.
	...
	virtual ~IVTabSwitchControl()
	{
		mTabLabels.Empty(true);
	}
	void Draw(IGraphics& g) override;
	void OnInit() override;
	...
	virtual void DrawButton(
		IGraphics& g, const IRECT& bounds, bool pressed, bool mouseOver, ETabSegment segment, bool disabled);
...
class IVKnobControl : public IKnobControlBase, public IVectorBase
{
 public:
	...
	void SetInnerPointerFrac(float frac)
	{
		mInnerPointerFrac = frac;
	}
	void SetOuterPointerFrac(float frac)
	{
		mOuterPointerFrac = frac;
	}
	void SetPointerThickness(float thickness)
	{
		mPointerThickness = thickness;
	}
...
class IVPlotControl : public IControl, public IVectorBase
{
 public:
	...
	IVPlotControl(const IRECT& bounds,
...
class IVGroupControl : public IControl, public IVectorBase
{
 public:
	...
	IVGroupControl(const char* label,
...
{
 public:
	IVPanelControl(const IRECT& bounds,
				   const char* label    = "",
...
class IVColorSwatchControl : public IControl, public IVectorBase
{
 public:
	...
	IVColorSwatchControl(const IRECT& bounds,
						 ...
						 const std::initializer_list<const char*>& labelsForIDs = {kVColorStrs[kBG],
																				   kVColorStrs[kFG],
																				   kVColorStrs[kPR],
																				   kVColorStrs[kFR],
																				   kVColorStrs[kHL],
...
class ISVGSwitchControl : public ISwitchControlBase
{
 public:
	 ...
	 * @param paramIdx The parameter index to link this control to */
	ISVGSwitchControl(const IRECT& bounds,
...
class ISVGSliderControl : public ISliderControlBase
{
 public:
	 ...
	 * @param gearing /todo */
	ISVGSliderControl(const IRECT& bounds,
...
class IBKnobControl : public IKnobControlBase, public IBitmapBase
{
 public:
	...
	IBKnobControl(const IRECT& bounds,
				  const IBitmap& bitmap,
				  int paramIdx,
				  EDirection direction = EDirection::Vertical,
				  double gearing       = DEFAULT_GEARING)
...
class IBSliderControl : public ISliderControlBase, public IBitmapBase
{
 public:
	...
	IBSliderControl(const IRECT& bounds,
					const IBitmap& handleBitmap,
					const IBitmap& trackBitmap = IBitmap(),
					int paramIdx               = kNoParameter,
					EDirection dir             = EDirection::Vertical,
