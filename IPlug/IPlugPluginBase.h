...
#pragma mark - Plug-in properties
	...
	{
		return mPluginName.Get();
	}

	/** Get the plug-in version number
	 * @param decimal Sets the output format
	 * @return Effect version in VVVVRRMM (if \p decimal is \c true) or Hexadecimal 0xVVVVRRMM (if \p decimal is \c
	 * false) format */
	 ...
	 * @note since the implementation is in IPlugAPIBase.cpp, you may want to touch that file as part of your build
	 * script to force recompilation
...
#pragma mark - State Serialization
	 ...
	 * parameters, into a binary byte chunk.
...
#pragma mark - Preset Manipulation
	 ...
	 * usage: MakePresetFromNamedParams(name, nParamsNamed, paramEnum1, paramVal1, paramEnum2, paramVal2, ...,
	 * paramEnumN, paramVal2) See DumpMakePresetFromNamedParamsSrc() which is a utility that can be used to create the
	 * code for MakePreset() calls nParamsNamed may be less than the total number of params.
	 * @param name The preset name
	...
	/** Creates a preset from an IByteChunk containging serialized data. This can be used when your plugin state
	 * includes arbitary data, other than just parameters.
	 * @param name The preset name
