...
#pragma mark -
	...
	{
		return math::Clamp((mFlags & kFlagStepped ? round(value / mStep) * mStep : value), mMin, mMax);
	}
	...
	{
		Set(FromNormalized(normalizedValue));
	}
	...
	/** Set the parameters label after creation. WARNING: if this is called after the host has queried plugin
	 * parameters, the host may display the label as it was previously
	...
	{
		return math::ToAmplitude(mValue.load());
	}
	...
	{
		return ToNormalized(mValue.load());
	}
	...
	{
		return mUnit == kUnitCustom ? mLabel : nullptr;
	}
	...
	{
		return normalized ? ToNormalized(GetDefault()) : mDefault;
	}
