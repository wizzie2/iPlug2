...
#pragma mark - Methods you implement in your plug-in class - you do not call these methods
	 ...
	 * (the maximum possible input channel count and the maximum possible output channel count including multiple
	 * buses). If the host hasn't connected all the pins, the unconnected channels will be full of zeros. THIS METHOD IS
	 * CALLED BY THE HIGH PRIORITY AUDIO THREAD - You should be careful not to do any unbounded, blocking operations
	 ...
	 * You can use IMidiQueue in combination with this method in order to queue the message and process at the
	 * appropriate time in ProcessBlock() THIS METHOD IS CALLED BY THE HIGH PRIORITY AUDIO THREAD - You should be
	 * careful not to do any unbounded, blocking operations such as file I/O which could cause audio dropouts
	 * @param msg The incoming midi message (includes a timestamp to indicate the offset in the forthcoming block of
	 * audio to be processed in ProcessBlock()) */
	 ...
	 * interpretations of "activate". Unlike OnReset() which called when the transport is reset or the sample rate
	 * changes OnActivate() is a good place to handle change of I/O connections.
...
#pragma mark -
	...
	/** For a given input or output bus what is the maximum possible number of channels. This method is not Realtime
	 * safe.
	 * @param direction Return input or output bus count
	...
	{
		return mChannelData[static_cast<int>(direction)].GetSize();
	}
	...
	{
		return (chIdx < mChannelData[static_cast<int>(direction)].GetSize() &&
				mChannelData[static_cast<int>(direction)].Get(chIdx)->mConnected);
	}
	...
	/** Check if a certain configuration of input channels and output channels is allowed based on the channel I/O
	 * configs
   ...
   * This may not be supported by the host. The method is virtual because it's overridden in API classes.
   @param latency Latency in samples */
	 ...
	 * ninchans-noutchans. A hypen character \c(-) deliminates input-output. Supports multiple buses, which are
	 * indicated using a period \c(.) character. For instance plug-in that supports mono input and mono output with a
	 * mono side-chain input could have a channel io string of "1.1-1". A drum synthesiser with four stereo output
	 * busses could be configured with a io string of "0-2.2.2.2";
...
#pragma mark - Methods called by the API class - you do not call these methods in your plug-in class
	...
	{
		return mChannelData[static_cast<int>(direction)].Get(idx)->mLabel;
	}
