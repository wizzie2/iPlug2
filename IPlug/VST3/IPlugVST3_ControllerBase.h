...
class IPlugVST3ControllerBase
{
 public:
	...
	void Initialize(IPlugAPIBase* pPlug, bool plugIsInstrument, bool midiIn)
	{
		...
		if constexpr (Config::UsePresetList)
		{
			if (pPlug->NPresets())
			{
				unitInfo.programListId = +EVST3ParamIDs::kPresetParam;
		...
		for (int i = 0; i < pPlug->NParams(); i++)
		{
			...
			if (CStringHasContents(paramGroupName))  // if the parameter has a group
			{
				...
				{
					if (strcmp(paramGroupName, pPlug->GetParamGroupName(j)) ==
						0)  // if group name found in existing groups
					{
						unitID = j + 1;  // increment unitID
					}
				}

				if (unitID == Steinberg::Vst::kRootUnitId)  // if unitID was still kRootUnitId, we found a new group, so
															// add it and add the unit
				{
		...
		assert(Config::Midi::NumberOfCCChannels <= Config::Midi::NumberOfMidiInChannels &&
			   "VST3_NUM_CC_CHANS must be less than or equal to VST3_NUM_MIDI_IN_CHANS");

		if constexpr (Config::Midi::NumberOfCCChannels > 0)
		{
			if (midiIn)
			{
				...
				unitInfo.parentUnitId  = Steinberg::Vst::kRootUnitId;
				unitInfo.programListId = Steinberg::Vst::kNoProgramListId;
				unitNameSetter.fromAscii(Config::Midi::CCUnitName);
				pEditController->addUnit(new Steinberg::Vst::Unit(unitInfo));
				...
				for (int chan = 0; chan < Config::Midi::NumberOfCCChannels; chan++)
				{
					...
					for (int i = 0; i < 128; i++)
					{
						Steinberg::UString(paramName, str16BufferSize(Steinberg::Vst::String128))
							.assign(IMidiMsg::GetCCNameStr(i));
	...
	bool SetChannelContextInfos(Steinberg::Vst::IAttributeList* pList)
	{
		...
		if (pList)
		{
			...
			if (pList->getInt(ChannelContext::kChannelNameLengthKey, length) == kResultTrue)
			{
				...
				if (pList->getString(ChannelContext::kChannelNameKey,
									 name.data(),
									 static_cast<Steinberg::uint32>(length + 1) * sizeof(TChar)) == kResultTrue)
				{
			...
			if (pList->getInt(ChannelContext::kChannelUIDLengthKey, length) == kResultTrue)
			{
				...
				// of byte: end of string zero value is written in an out of memory position
				std::vector<TChar> name((length + 1) * 2);
				if (pList->getString(ChannelContext::kChannelUIDKey,
									 name.data(),
									 static_cast<Steinberg::uint32>(length + 1) * sizeof(TChar)) == kResultTrue)
				{
					Steinberg::String str(name.data());
			...
			if (pList->getInt(ChannelContext::kChannelIndexNamespaceLengthKey, length) == kResultTrue)
			{
				...
				// of byte: end of string zero value is written in an out of memory position
				std::vector<TChar> name((length + 1) * 2);
				if (pList->getString(ChannelContext::kChannelIndexNamespaceKey,
									 name.data(),
									 static_cast<Steinberg::uint32>(length + 1) * sizeof(TChar)) == kResultTrue)
				{
					Steinberg::String str(name.data());
