...
namespace iplug
{
	...
	{  // clang-format off
		...
		// TODO: add option to set values from cmake configuration (collect all options first)
		// Perhaps move the whole struct to an cmake autogenerated file instead of a bunch of #defines?
		...
		static constexpr      size_t defaultMidiQueueBlockSize = 512;
		static constexpr      size_t defaultChannelBlockSize   = 1024;  // Array size of each processing in and out channel
		...
		struct Midi
		{
			...
			static constexpr         int SysExArraySize           = 512;
			static constexpr const char* CCUnitName               = VST3_CC_UNITNAME;
			static constexpr         int NumberOfCCChannels       = VST3_NUM_CC_CHANS;
			static constexpr         int NumberOfMidiInChannels   = VST3_NUM_MIDI_IN_CHANS;
			static constexpr         int NumberOfMidiOutChannels  = VST3_NUM_MIDI_OUT_CHANS;
		};

		// Mixed debug settings (Should preferably be able to change these without affecting source controlled files in the end)
		struct Debug
		{
			static constexpr   bool DebugBuild                     = (EBuildType::Native == EBuildType::Debug);
			static constexpr   bool ShowVBlankDelayedQueueMessages = true;
			static constexpr   bool ShowVBlankMessages             = DebugBuild;
			static constexpr   bool MidiSynthShowVoiceCount        = false;
		};
	...
	struct SysExData
	{
		...
		{
			assert(size < Config::Midi::SysExArraySize);
